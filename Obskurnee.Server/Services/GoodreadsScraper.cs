...
public class GoodreadsScraper : IExternalBookScraper
{
        ...
        string bookPageHtml)
    {
        ...
        if (!string.IsNullOrWhiteSpace(result.Title))
            result.Title = WebUtility.HtmlDecode(result.Title);

        if (!string.IsNullOrWhiteSpace(result.Author))
            result.Author = WebUtility.HtmlDecode(result.Author);

        if (!string.IsNullOrWhiteSpace(result.Description))
            result.Description = WebUtility.HtmlDecode(result.Description);
